...
def money(N, numwidth=0, currency='$'):
    '''
    Format number N for display with commas, 2 decimal
    digits, leading $ and sign, and optional padding:
    '$ -xxx,yyy.zz'.
    numwidth=0 for no space padding, currency='' to omit
    symbol, and non-ASCII for others.
    (e.g., pound=u'\xA3' or u'\u00A3).
    '''
    sign = '-' if N < 0 else ''
    N = abs(N)
    whole = commas(int(N))
